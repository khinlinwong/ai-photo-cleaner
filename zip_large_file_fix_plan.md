...
- **架构决策**：不建议继续单纯增加 `setTimeout` 的延迟时间，也不建议继续在前端环境中追求直接生成单个 1.2GB+ 的大 Blob ZIP 包。
- **下一步行动**：建议立刻进入 `CORE-ZIP-BATCH-EXPORT-PLANNING` 阶段。后续优先规划按照片张数（例如每 50 张一包）或按包体积限制（例如每 500MB 一包）分批导出多个 ZIP 压缩包，从源头上避免产生 1GB+ 的超大 Blob 对象。
- **长期技术方向**：在后续的稳定版本中，通过 Web Worker、流式压缩或 Tauri 原生本地打包机制来解决大型文件的持久化导出。
- **防污染约束**：在大文件 ZIP 导出缺陷被彻底解决前，USE_SIGNAL_GROUPS_FOR_BATTLE 开关继续保持默认 false，不改变 ZIP 分区逻辑及 Photo Battle 状态。
