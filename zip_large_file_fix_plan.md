...
在 `CORE-ZIP-LARGE-FILE-FIX` 阶段，已执行了此最小修复：
- **实现方式**：将 `URL.revokeObjectURL(downloadUrl)` 从立即同步释放，改为包裹在 `setTimeout` 中执行 120 秒延迟释放。
- **定位与边界**：这是针对浏览器大 Blob 异步磁盘写入的短期原型修复方案，而非长期的最佳大文件导出架构。
- **回归测试结论**：已在开发环境中执行 100 张与 200 张大图回归测试。实测结果表明，100 张淘汰区 ZIP（643.25MB）与 200 张保留区 ZIP（15.78MB）成功完整下载，不再发生中断；但 200 张淘汰区大 ZIP（1.27GB）在延时释放情况下，依然触发 `DownloadInterrupted` 错误。这表明仅延迟 Object URL 的生命周期无法应对 1.2GB 级别超大文件由于内存飙升（OOM）或无头浏览器数据通道限制引起的下载失败。下一步仍需考虑升级到 pending URLs 精细化生命周期管理或长期流式打包方案。
...
- **实测结论**：回归测试已证实，在 200 张淘汰候选区大包（1.27GB）场景下，延迟 120 秒释放依然发生了 `DownloadInterrupted` 中断。
- **局限性**：延迟 revoke 仅能够消除中等体积包（如 100 张 643MB）因立即释放导致的中断，但无法解决超大单包 Blob（>1.2GB）在浏览器单线程上由于内存消耗过高（OOM）或数据通道传输极限导致的物理阻断。
- **架构决策与实现**：不建议继续单纯增加 `setTimeout` 的延迟时间，也不建议继续在前端环境中追求直接生成单个 1.2GB+ 的大 Blob ZIP 包。因此，项目已在 `CORE-ZIP-BATCH-EXPORT` 阶段完整实现了分批 ZIP 导出方案，并在 `CORE-ZIP-BATCH-EXPORT-REGRESSION` 阶段成功通过了回归测试。
- **分批 ZIP 验证与定位**：在当前 100 张和 200 张大尺寸 JPG 物理图片样本测试下，分批 ZIP 导出方案彻底补上了大包下载中断的缺口。200 张淘汰候选区在当前测试环境里成功通过自动分包（分成 3 个 part，单包最大约 247MB，合计 120 张）以及 1500ms 串行延迟下载，不再生成 1GB+ 的超大单包，也不再出现 `DownloadInterrupted` 错误。
- **长期技术方向**：该分批 ZIP 是针对前端浏览器大文件导出瓶颈的短期规避方案。由于测试表现受具体软硬件与浏览器环境限制，我们并不将其视作在所有未知生产环境下的永久万能解决方案。后续我们仍然保留 Web Worker 后台压缩、流式 ZIP 边压边下以及 Tauri 原生 Rust 本地文件系统打包操作作为解决大文件持久导出的长期核心技术方向。
- **防污染约束**：在分批 ZIP 方案完全通过回归测试的前提下，USE_SIGNAL_GROUPS_FOR_BATTLE 开关依然在代码中继续强力保持默认 false，不改变 ZIP 分区筛选算法或 Photo Battle 状态。
