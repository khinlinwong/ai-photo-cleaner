...
- 正式主流程仍保持 legacy。
- **CORE-DUPLICATE-11-PLANNING 进展更新**：已规划真实 100-300 张物理图片文件的 true 分支压力测试，详细方案参阅项目根目录下的 [duplicate_real_file_stress_test_plan.md](file:///C:/Users/khinl/Documents/AI%20Photo%20Cleaner/duplicate_real_file_stress_test_plan.md)。200 张元数据仿真测试通过后，不能直接代表真实图片读取、解码及 Canvas 的物理压力，后续执行仍需遵循“即测即恢复 false”的红线。
- **CORE-DUPLICATE-11 真实 100/200/300 张 BMP 文件测试已通过**：已成功完成真实 BMP 文件读取和 Canvas 分析链路测试。本次测试覆盖了物理文件读取、Canvas 灰度计算及 JSZip 物理大文件打包测试。测试在 300 张以内（总 108MB 数据量）表现流畅，但该结果仍不能代表 JPG / PNG / WebP 压缩解码压力，也不代表 HEIC / RAW 及 10MB 手机大图的表现。
